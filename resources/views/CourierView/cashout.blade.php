...
@section('content')
    ...
    <form action="" method="POST">
        ...
        <br><br>
        Available amount: <input type="number" name="availableAmount" placeholder="{{$courier->due_delivery_fee}}" value=" {{$courier->due_delivery_fee}} " readonly>
        <br><br>
        Cashout amount: <input type="number" name="amount" placeholder="{{$courier->due_delivery_fee}}" value=" {{$courier->due_delivery_fee}} ">
        <br><br>
        @error('due')
            {{ $message}}<br>
        @enderror
